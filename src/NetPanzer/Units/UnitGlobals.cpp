...
void colorMapGray(PackedSurface &source, PackedSurface &dest,
                  ColorTable &colorTable) {
  // colormapping gray scheme.
  Surface tempSurface;

  tempSurface.create(source.getWidth(), source.getHeight(),
  ...
  for (int i = 0; i < source.getFrameCount(); i++) {
    ...
    tempSurface.bltLookup(
        iRect(0, 0, tempSurface.getWidth(), tempSurface.getHeight()),
