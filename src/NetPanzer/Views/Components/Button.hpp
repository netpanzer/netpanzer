...
typedef enum { BNORMAL = 0, BOVER = 1, BPRESSED = 2 } ButtonState;
...
class Button : public Component {
 ...
 public:
  ...
  static Button *createTextButton(std::string cname, std::string label, iXY loc,
                                  int bwidth);

  static Button *createSpecialButton(std::string cname, std::string label,
                                     iXY loc);
  ...
  void setTextButtonSize(int xsize) {
    Component::setSize(xsize + (extraBorder * 2),
                       Surface::getFontHeight() + 4 + (extraBorder * 2));
  }
