...
class cInputFieldString : public NoCopy {
 ...
 public:
  cInputFieldString() { reset(); }

  ~cInputFieldString() { free(); }
...
class cInputField {
 public:
  ...
  iXY getPos() const { return pos; }
  int getMaxCharCount() const { return maxCharCount; }
  char *getDestString() const { return destString; }
  char *getExcludedCharacters() const { return excludedCharacters; }

  bool contains(const iXY &pos) const { return bounds.contains(pos); }
  ...
  void setmaxChar(const int maxChar) { maxCharCount = maxChar; }
