...
class View : public iRect {
 ...
 public:
  typedef std::vector<Component *>
      ComponentList;  // switched to vector instead of list because memory is
                      // stored in continuous block, faster lookup.
  ...
  int getPressedButton() { return pressedButton; }
  int getPrevPressedButton() { return prevPressedButton; }
  int getHighlightedButton() { return highlightedButton; }
  int getPrevHighlightedButton() { return prevHighlightedButton; }
 ...
 protected:
  ...
  // cButton Functions.
  void addButtonCenterText(const iXY &pos, const int &xSize, const char *nName,
  ...
  cInputField *addInputField(const iXY &pos, cInputFieldString *string,
                             ...
                             const bool &isSelected, const int maxCharCount);
  ...
  virtual void lMouseDrag(const iXY &downPos, const iXY &prevPos,
                          const iXY &newPos);
  ...
  inline void resize(const int &x, const int &y) { resize(iXY(x, y)); }
 ...
 public:
  ...
  inline void moveTo(const int &x, const int &y) { moveTo(iXY(x, y)); }
