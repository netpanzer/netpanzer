...
// setSize
//---------------------------------------------------------------------------
void GameView::setSize(iXY size) { resize(size); }  // end GameView::setSize
...
void GameView::checkResolution(iXY oldResolution, iXY newResolution) {
...
void GameView::drawMap(Surface &window) {
  TileSet *ts = TileInterface::getTileSet();
  iXY world;
  iXY map;

  WorldViewInterface::getMainCamera()->getViewStart(
      window.getWidth(), window.getHeight(), &world.x, &world.y);
