...
static void bNextMap() {
  ...
  GameConfig::game_mapcycle->assign(
      MapSelectionView::mapList[MapSelectionView::curMap]->name);
...
MapSelectionView::MapSelectionView() : RMouseHackView() {
  ...
  resizeClientArea(bodyTextRect.getSizeX() / 2 - 10 + 30,
                   MAP_SIZE + BORDER_SPACE * 2);
...
MapSelectionView::~MapSelectionView() {
  ...
  for (i = mapList.begin(); i != mapList.end(); i++) delete *i;
}
...
void MapSelectionView::init() {
  const int arrowButtonWidth =
      (getClientRect().getSizeX() - MAP_SIZE - BORDER_SPACE * 3) / 2;

  iXY pos(MAP_SIZE + BORDER_SPACE * 2,
          getClientRect().getSizeY() - Surface::getFontHeight() - BORDER_SPACE);
...
int MapSelectionView::loadMaps() {
  ...
  for (unsigned int i = 0; i < mapfiles.size(); i++) {
    try {
      ...
      LOGGER.warning("cannot open map file '%s': %s", mapfiles[i].c_str(),
                     e.what());
      continue;
    }
  }
