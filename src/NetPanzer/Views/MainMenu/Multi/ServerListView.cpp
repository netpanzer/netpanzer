...
ServerListView::ServerListView() : queryThread(0) {
  setSearchName("ServerListView");
  setTitle("Servers");
  setSubTitle(
      " Name                         Players/max     Map          Ping   ");
...
void ServerListView::buttonRefresh() { serverlistview->refresh(); }

masterserver::ServerList ServerListView::getViewModel() {
  ...
  for (masterserver::ServerInfo* serverInfoPtr : serverlist) {
    if (serverInfoPtr->protocol < NETPANZER_PROTOCOL_VERSION) {
      ...
      // But we don't want to punish players for running a newer version by
      // cluttering the server list. So if server is too old - just ignore it.
    } else {
...
void ServerListView::doDraw(Surface& windowArea, Surface& clientArea) {
  ...
  for (masterserver::ServerInfo* serverPtr : viewModel) {
    ...
    if (y >= clientArea.getHeight()) break;
  }

  View::doDraw(windowArea, clientArea);
}

int ServerListView::lMouseUp(const iXY& down_pos, const iXY& up_pos) {
