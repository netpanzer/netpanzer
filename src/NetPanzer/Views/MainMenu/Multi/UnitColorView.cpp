...
static void setGrayColorTable() { colorTable = &gray256; }
...
static void rebuildGrayColorTable() {
  ...
  for (int num = 0; num < 256; num++) {
    int c = int(Palette::color[num].r + Palette::color[num].g +
                Palette::color[num].b) /
            3;  // brightness

    int nearestColor = Palette::findNearestColor(
        int(c * grayPercent), int(c * grayPercent), int(c * grayPercent));
...
void UnitColorView::doDraw(Surface &viewArea, Surface &clientArea) {
  ...
  fuckingSurface.bltLookup(fuckingSurface.getRect(),
...
//---------------------------------------------------------------------------
void UnitColorView::rMouseDown(const iXY &pos) { View::lMouseDown(pos); }
...
//---------------------------------------------------------------------------
void UnitColorView::rMouseDrag(const iXY &downPos, const iXY &,
                               const iXY &newPos) {
