...
static int getCurMaxPlayersCount() { return GameConfig::game_maxplayers; }

static int getCurMaxUnitCount() { return GameConfig::game_maxunits; }
...
void HostOptionsView::updateGameConfigCloudCoverage() {
  ...
  float cloudCount =
      MapSelectionView::mapList[MapSelectionView::curMap]->cells.getArea() /
      baseTileCountPerCloud;
  ...
  float fuckingCloudyCloudCount =
      float(cloudCount) * extremelyCloudyPercentOfBase;
...
static int getTimeLimitHours() { return GameConfig::game_timelimit / 60; }

static int getTimeLimitMinutes() { return GameConfig::game_timelimit % 60; }
...
static int getFragLimit() { return GameConfig::game_fraglimit; }
...
void HostOptionsView::addMeterButtons(const iXY &pos) {
  ...
  x = xTextStart;
  add(new Label(x, y, "Max Players", windowTextColor, windowTextColorShadow,
                true));
  x += xControlStart;
  addButtonCenterText(iXY(x - 1, y), arrowButtonWidth, "<", "",
                      bDecreasePlayerCount);
  x += arrowButtonWidth + meterWidth;
  addButtonCenterText(iXY(x + 1, y), arrowButtonWidth, ">", "",
                      bIncreasePlayerCount);
  y += yOffset;

  x = xTextStart;
  add(new Label(x, y, "Game Max Unit Count", windowTextColor,
                windowTextColorShadow, true));
  x += xControlStart;
  addButtonCenterText(iXY(x - 1, y), arrowButtonWidth, "<", "",
                      bDecreaseMaxUnitCount);
  x += arrowButtonWidth + meterWidth;
  addButtonCenterText(iXY(x + 1, y), arrowButtonWidth, ">", "",
                      bIncreaseMaxUnitCount);
  y += yOffset;
  /*
      x = xTextStart;
      //addLabelShadowed(iXY(x, y), "Player Respawn Unit Count",
     windowTextColor, windowTextColorShadow); x += xControlStart;
      addButtonCenterText(iXY(x - 1, y), arrowButtonWidth, "<", "",
     bDecreaseRespawnCount); x += arrowButtonWidth + meterWidth;
      addButtonCenterText(iXY(x + 1, y), arrowButtonWidth, ">", "",
     bIncreaseRespawnCount); y += yOffset;
  */
  // x = xTextStart;
  // addLabelShadowed(iXY(x, y), "Allow Allies", windowTextColor,
  // windowTextColorShadow); x += xControlStart; addButtonCenterText(iXY(x - 1,
  // y), arrowButtonWidth, "<", "", bSetAllowAlliesFalse); x += arrowButtonWidth
  // + meterWidth; addButtonCenterText(iXY(x + 1, y), arrowButtonWidth, ">", "",
  // bSetAllowAlliesTrue); y += yOffset;
  //
  // x = xTextStart;
  // addLabelShadowed(iXY(x, y), "Spawn Placement", windowTextColor,
  // windowTextColorShadow); x += xControlStart; addButtonCenterText(iXY(x - 1,
  // y), arrowButtonWidth, "<", "", bPreviousSpawnPlacement); x +=
  // arrowButtonWidth + meterWidth; addButtonCenterText(iXY(x + 1, y),
  // arrowButtonWidth, ">", "", bNextSpawnPlacement); y += yOffset;

  x = xTextStart;
  add(new Label(x, y, "Objective Capture Percent", windowTextColor,
                windowTextColorShadow, true));
  x += xControlStart;
  addButtonCenterText(iXY(x - 1, y), arrowButtonWidth, "<", "",
                      bDecreaseObjectiveCapturePercent);
  x += arrowButtonWidth + meterWidth;
  addButtonCenterText(iXY(x + 1, y), arrowButtonWidth, ">", "",
                      bIncreaseObjectiveCapturePercent);
  y += yOffset;

  x = xTextStart;
  add(new Label(x, y, "Time Limit", windowTextColor, windowTextColorShadow,
                true));
  x += xControlStart;
  addButtonCenterText(iXY(x - 1, y), arrowButtonWidth, "<", "",
                      bDecreaseTimeLimit);
  ...
  x = xTextStart;
  add(new Label(x, y, "Frag Limit", windowTextColor, windowTextColorShadow,
                true));
  x += xControlStart;
  addButtonCenterText(iXY(x - 1, y), arrowButtonWidth, "<", "",
                      bDecreaseFragLimit);
  ...
  int checkboxesPosY =
      y + choiceGameType.getSize().y + Surface::getFontHeight();
  ...
  choiceMapStyle.setLocation(
      358, y + choiceWindSpeed.getSize().y + Surface::getFontHeight());
  choiceMapStyle.select(getMapStyleString());
  add(&choiceMapStyle);

  /*
      x = xTextStart;
      //addLabelShadowed(iXY(x, y), "Allow Fog Of War", windowTextColor,
     windowTextColorShadow); x += xControlStart; addButtonCenterText(iXY(x - 1,
     y), arrowButtonWidth, "<", "", bSetAllowFogOfWarFalse); x +=
     arrowButtonWidth + meterWidth; addButtonCenterText(iXY(x + 1, y),
     arrowButtonWidth, ">", "", bSetAllowFogOfWarTrue);
  */
...
void HostOptionsView::drawMeterInfo(Surface &dest, const iXY &pos) {
  ...
  /*
      ...
      tempSurface.fill(meterColor);
      tempSurface.drawButtonBorder(meterTopLeftBorderColor,
     meterBottomRightBorderColor); sprintf(strBuf, "%d", getRespawnUnitCount());
  ...
  */
  ...
  // tempSurface.fill(meterColor);
  // tempSurface.drawButtonBorder(meterTopLeftBorderColor,
  // meterBottomRightBorderColor); sprintf(strBuf, "%s", getAllowAllies());
  // tempSurface.bltStringCenter(strBuf, meterTextColor);
  // tempSurface.blt(dest, x, y);
  //
  //// Spawn Placement
  ...
  // tempSurface.fill(meterColor);
  // tempSurface.drawButtonBorder(meterTopLeftBorderColor,
  // meterBottomRightBorderColor); sprintf(strBuf, "%s", getSpawnPlacement());
  // tempSurface.bltStringCenter(strBuf, meterTextColor);
  // tempSurface.blt(dest, x, y);
  ...
  if (MapSelectionView::curMap >= 0 && MapSelectionView::mapList.size() > 0) {
    ...
    sprintf(strBuf, "%d%% - %d of %d", getObjectiveCapturePercent(),
            int(float(objectiveCount) *
                    (float(getObjectiveCapturePercent()) / 100.0f) +
                0.999),
            objectiveCount);
  } else {
  ...
  /*
      ...
      tempSurface.fill(meterColor);
      tempSurface.drawButtonBorder(meterTopLeftBorderColor,
     meterBottomRightBorderColor); sprintf(strBuf, "%s", getAllowFogOfWar());
      tempSurface.bltStringCenter(strBuf, meterTextColor);
      tempSurface.blt(dest, x, y);
  */
