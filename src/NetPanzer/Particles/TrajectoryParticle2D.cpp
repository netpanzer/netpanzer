...
//---------------------------------------------------------------------------
TrajectoryParticle2D::TrajectoryParticle2D(const fXYZ &pos, int maxSpeed,
  ...
  // Calculate the lifetime of this particle based off of the angle of
  // trajectory and the initialVelocity of the particle.

  // This makes the particle die when it reaches its peak height.
  // lifetime = (float(initialVelocity * sin(trajectoryAngle)) /
  // Physics::getGravity());
...
// sim
//---------------------------------------------------------------------------
// Purpose: Handles the particle simulation of a trajectory particle, no
// drawing.
//---------------------------------------------------------------------------
void TrajectoryParticle2D::sim() {
  ...
  // Move the particle in the correct direction based off the direction the
  // particle is facing and the horizontal placement the particle has
  // experienced.
  pos.x = initialPos.x + dx * direction.x;
  pos.z = initialPos.z + dx * direction.z;

  pos.y =
      Physics::getVerticalPosition(initialVelocity, int(trajectoryAngle), age);
