...
// FlameParticle2D
//---------------------------------------------------------------------------
FlameParticle2D::FlameParticle2D(const fXYZ& pos, float scaleMin,
                                 float scaleRand, float lifetime, int layer)
    : Particle2D(pos) {
  if (lifetime <= 0.0f) {
    isAlive = false;
  }
...
void FlameParticle2D::loadPakFiles() {
  ...
  if (!loadAllPAKInDirectory(pathExplosion0, staticPackedExplosion0)) {
    throw Exception("ERROR: Unable to load any exposion images in %s",
                    pathExplosion0);
  }
...
// init
//---------------------------------------------------------------------------
void FlameParticle2D::init() { loadPakFiles(); }  // end FlameParticle2D::init
