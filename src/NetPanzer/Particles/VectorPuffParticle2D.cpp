...
VectorPuffParticle2D::VectorPuffParticle2D(
    const fXYZ &pos, const fXYZ &direction, float scaleMin, float scaleRand,
    float speedMin, float speedRand, float waitMin, float waitRand,
    float lifetimeMin, float lifetimeRand, float windScale)
  ...
  velocity =
      direction * ((float(rand()) / float(RAND_MAX)) * speedRand + speedMin);
...
void VectorPuffParticle2D::sim() {
  ...
  if (waitTime > wait) {
    ...
    if (particleType == 0) {
      try {
        new PuffParticle2D(pos, LIGHT, scaleMin, scaleRand, smokeFPSMin,
      ...
      try {
        new PuffParticle2D(pos, DARK, scaleMin, scaleRand, smokeFPSMin,
                           smokeFPSRand, smokeLayer, smokeShadowLayer,
                           windScale, 0);
      } catch (...) {
      }
