...
void ParticleInterface::addDirtPuffParticle(const iXY &) {
  //	addPuffParticle(worldPos, DIRT_PUFF, smolderMinScale, smolderRandScale,
  //smolderMinFPS, smolderRandFPS, smolderLayer);
}
...
void ParticleInterface::addDirtPuffSystem(const iXY &worldPos,
  ...
  (void)worldPos;
  (void)bounds;
  //	addPuffSystem(worldPos, bounds, dirtPuffMaxParticleCount, DIRT_PUFF,
  //smolderMinScale, smolderRandScale, smolderMinFPS, smolderRandFPS,
  //smolderLayer);
}
...
void ParticleInterface::addUnitDamagePuffParticle(const UnitState &unitState) {
  ...
  if (particleType == 0) {
    try {
      new PuffParticle2D(newPos, LIGHT, 0.0f, 0.4f, smokeFPSMin, smokeFPSRand,
    ...
    try {
      new PuffParticle2D(newPos, DARK, 0.0f, 0.4f, smokeFPSMin, smokeFPSRand,
                         smokeLayer, smokeShadowLayer, smokeWindScale,
                         Particle2D::getFarAway(newPos));
    } catch (...) {
      // ignored for now
...
//--------------------------------------------------------------------------
void ParticleInterface::addSmokePuffParticle(
    const iXY &worldPos, float minScale, float randScale, int minFPS,
    int randFPS, int layer, float windScale /* = 1.0f */,
    float incrementScale /* = 0.0f */) {
  int particleType = rand() % 2;

  if (particleType == 0) {
    addPuffParticle(worldPos, LIGHT, minScale, randScale, minFPS, randFPS,
                    layer, windScale, incrementScale);
  } else if (particleType == 1) {
    addPuffParticle(worldPos, DARK, minScale, randScale, minFPS, randFPS, layer,
                    windScale, incrementScale);
...
void ParticleInterface::addSmokePuffParticle(const iXY &worldPos,
  ...
  (void)worldPos;
  (void)type;
  //	addPuffParticle(worldPos, type, smolderMinScale, smolderRandScale,
  //smolderMinFPS, smolderRandFPS, smolderLayer, smolderWindScale,
  //smolderIncrementScale);
}

//--------------------------------------------------------------------------
void ParticleInterface::addPuffParticle(const iXY &worldPos, PUFF_TYPE type,
                                        float minScale, float randScale,
                                        int minFPS, int randFPS, int layer,
...
//--------------------------------------------------------------------------
void ParticleInterface::addSmokePuffSystem(const iXY &worldPos,
                                           const iRect &bounds,
  ...
  //    if      (particleType == 0)
  //    {
  //		addPuffSystem(worldPos, bounds, maxParticleCount, LIGHT,
  //smolderMinScale, smolderRandScale, smolderMinFPS, smolderRandFPS,
  //smolderLayer);
  //    }
  //    else if (particleType == 1)
  //    {
  //		addPuffSystem(worldPos, bounds, maxParticleCount, DARK,
  //smolderMinScale, smolderRandScale, smolderMinFPS, smolderRandFPS,
  //smolderLayer);
  //    }
}
...
//--------------------------------------------------------------------------
void ParticleInterface::addPuffSystem(const iXY &worldPos, const iRect &bounds,
                                      int maxParticleCount,
  ...
  //
  //    int particleCount = (rand() % maxParticleCount) + 1; // max
  //    maxParticleCount, min 1
  //
...
void ParticleInterface::addVehicleExplosionFlameParticle(const iXY &worldPos) {
  (void)worldPos;
  //	addExplosionFlameParticle(worldPos, vehicleExplosionFlameMinScale,
  //vehicleExplosionFlameRandScale, vehicleExplosionFlameLayer);
}

//--------------------------------------------------------------------------
void ParticleInterface::addVehicleExplosionFlashParticle(const iXY &worldPos) {
  (void)worldPos;
  //	addFlashParticle(worldPos, vehicleExplosionFlashScaleMin,
  //vehicleExplosionFlashScaleRand, vehicleExplosionFlashLifetime,
  //vehicleExplosionFlashLayer);
}

//--------------------------------------------------------------------------
void ParticleInterface::addMissleExplosionFlameParticle(const iXY &worldPos) {
  (void)worldPos;
  //	addExplosionFlameParticle(worldPos, missleExplosionFlameMinScale,
  //missleExplosionFlameRandScale, missleExplosionFlameLayer);
}

//--------------------------------------------------------------------------
void ParticleInterface::addMissleExplosionFlashParticle(const iXY &worldPos) {
  (void)worldPos;
  //	addFlashParticle(worldPos, missleExplosionFlashScaleMin,
  //missleExplosionFlashScaleRand, missleExplosionFlashLifetime,
  //missleExplosionFlashLayer);
}

//--------------------------------------------------------------------------
void ParticleInterface::addFlashParticle(const iXY &worldPos, float minScale,
                                         float randScale, float lifetime,
                                         int layer,
                                         bool singleFrame /* = false */) {
  try {
    new FlashParticle2D(fXYZ(worldPos.x, 0, worldPos.y), minScale, randScale,
                        lifetime, layer, singleFrame);
  } catch (...) {
    // ignrored
  }
}
...
//--------------------------------------------------------------------------
void ParticleInterface::addExplosionFlameParticle(const iXY &worldPos,
                                                  const float &minScale,
                                                  const float &randScale,
                                                  const int &layer) {
  (void)worldPos;
  (void)minScale;
  (void)randScale;
...
//--------------------------------------------------------------------------
void ParticleInterface::addExplosionSmokeParticle(const iXY &worldPos,
                                                  int maxParticleSpeed) {
  (void)worldPos;
...
//--------------------------------------------------------------------------
void ParticleInterface::addExplosionDirtSystem(const iXY &worldPos,
                                               const iRect &bounds) {
  (void)worldPos;
  (void)bounds;
...
//--------------------------------------------------------------------------
void ParticleInterface::addGroundExplosionSystem(const iXY &worldPos,
                                                 const iRect &bounds,
                                                 int maxParticleCount,
                                                 int maxParticleSpeed) {
  (void)worldPos;
...
//--------------------------------------------------------------------------
void ParticleInterface::addExplosionSmokeSystem(const iXY &worldPos,
                                                const iRect &bounds,
                                                int maxParticleCount,
                                                int maxParticleSpeed) {
  (void)worldPos;
  ...
  */
  ...
  // Add an additional set of particles in the center of the larger explosion
  // which move at a slower initialVelocity, since real explosions have a lot of
  // smoke lingering in the middle of the smoke cloud.
  int slowerParticleSpeed = maxParticleSpeed - 1;

  if (slowerParticleSpeed > 0) {
    try {
      // new ExplosionParticleSystem2D(pos, bounds, maxParticleCount / 2,
      // slowerParticleSpeed, particleType);
    } catch (...) {
    }
  }
}
...
//--------------------------------------------------------------------------
void ParticleInterface::addSmolderSystem(const iXY &worldPos,
                                         const iRect &bounds, float lifetime,
                                         float percent) {
  ...
  if (particleType == 0) {
    try {
      new SmolderParticleSystem2D(pos, bounds, lifetime, waitMin, waitMax,
                                  LIGHT);
    } catch (...) {
    }
  } else if (particleType == 1) {
...
//--------------------------------------------------------------------------
void ParticleInterface::addCloudParticle(const iXY &worldPos,
                                         const iXY &worldSize) {
  (void)worldSize;
...
void ParticleInterface::addCloudParticleSystem(int maxParticleCount) {
  ...
  for (int i = 0; i < maxParticleCount; i++) {
    addCloudParticle(iXY(rand() % shit.x, rand() % shit.y),
                     iXY(shit.x, shit.y));
  }
}
...
void ParticleInterface::addHit(const UnitState &unitState) {
  ...
  e.flashCount =
      int(3 * float(unitParticleInfo[unitState.unit_type].minBounds.getArea()) /
  ...
  //
  //	float percentScale = (float(rand()) / float(RAND_MAX)) * randScale +
  //minScale;
  //
  //	PUFF_TYPE puffType;
  //
  ...
  //
  //             //addSmolderSystem(worldPos, iRect(-5, -5, 5, 5), smolderLife,
  //             smolderPercent);
...
float ParticleInterface::getFrameRateAdjustment() {
  ...
  float adjustment =
      (TimerInterface::getFPS() - frameMinbase) / frameDifference;
...
void ParticleInterface::addMiss(const iXY &worldPos, Uint8 unitType) {
  ...
  */
  ...
  e.particleCount =
      int(5 * (frameRateAdjustment * float(unitAttackFactorTable[unitType]) *
               randomness));
  ...
  try {
    new TemplateExplosionSystem(pos, iRect(-5, -5, 5, 5), e,
...
//--------------------------------------------------------------------------
void ParticleInterface::addMissleFlightPuff(const iXY &worldPos,
                                            const fXY &direction,
                                            float &curWait, float &totalWait,
                                            Uint8 unitType) {
  (void)unitType;
  ...
  if (curWait > totalWait) {
    ...
    try {
      new PuffParticle2D(loc, LIGHT, 0.05f, 0.05f, smokeFPSMin, smokeFPSRand,
                         smokeLayer, smokeShadowLayer, MuzzleSystem::windScale,
...
//--------------------------------------------------------------------------
void ParticleInterface::addMissleLaunchPuff(const iXY &worldPos,
                                            const fXY &direction,
                                            Uint8 unitType) {
  (void)worldPos;
  (void)direction;
  ...
  //	fXYZ pos(worldPos.x, 0, worldPos.y);
  //
  //	new VectorPuffParticle2D(pos, -direction, missleLaunchPuffMinScale,
  //missleLaunchPuffRandScale, missleLaunchPuffSpeedMin,
  //missleLaunchPuffSpeedRand, missleLaunchPuffWaitMin,
  //missleLaunchPuffWaitRand, missleLaunchPuffLifetimeMin,
  //missleLaunchPuffLifetimeRand, missleLaunchPuffWindScale);
  //	//for (int i = 0; i < 10; i++)
  //	//{
  //		//const int radius = 10;
  //
  ...
  //		//addSmokePuffParticle(pos, missleLaunchPuffMinScale,
  //missleLaunchPuffRandScale, missleLaunchPuffMinFPS, missleLaunchPuffRandFPS,
  //missleLaunchPuffLayer);
  //	//}
  //	//}
}
  ...
  for (int i = 0; i < count; i++) {
    try {
      new CloudParticle2D(fXYZ(rand() % shit.x, 0, rand() % shit.y),
                          iXY(shit.x, shit.y), cloudWindMinPercent,
                          cloudWindRandPercent);
    } catch (...) {
    }
  }
...
void ParticleInterface::addMoveDirtPuff(const UnitState &unitState) {
  ...
  if (TileInterface::getWorldPixMovementValue(unitState.location.x,
    ...
    if (movePuffWaitGroup >= movePuffWaitTotal) {
      ...
      try {
        new PuffParticle2D(pos, DIRT, 0.0f, 0.15f, (int)(smokeFPSMin * 0.75f),
                           (int)(smokeFPSRand * 0.75f), 1, 0,
                           MuzzleSystem::windScale, isFarAway);
      } catch (...) {
        // ignored for now
      }
    }
...
void ParticleInterface::getUnitParticleInfo() {
  ...
  for (unsigned short i = 0; i < UnitProfileInterface::getNumUnitTypes(); i++) {
    ...
    UnitProfileSprites *uprofile =
        UnitProfileSprites::getUnitProfileSprites(vector_index);
...
//--------------------------------------------------------------------------
void ParticleInterface::getMuzzleTips(PackedSurface &packedSource,
                                      iXY muzzleTips[36]) {
  // Copy all the packed images onto regular surfaces.
  Surface source(packedSource.getWidth(), packedSource.getHeight(),
                 packedSource.getFrameCount());
  {
  ...
  // Frame 27 - 35
  {
    for (int i = 27; i < 36; i++) {
      ...
      for (unsigned int offset = 0; offset < (unsigned int)source.getCenterX();
           offset++) {
        int yy = source.getHeight() - 1 - offset;
        for (unsigned int x = offset; x < source.getWidth(); x++) {
          if (source.getPixel(x, yy) != 0) {
...
// getMinBounds
//--------------------------------------------------------------------------
void ParticleInterface::getMinBounds(PackedSurface &packedSource,
                                     iRect &minBounds) {
  // Copy all the packed images onto regular surfaces.
  Surface source(packedSource.getWidth(), packedSource.getHeight(),
                 packedSource.getFrameCount());
