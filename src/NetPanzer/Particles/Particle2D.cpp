...
void *Particle2D::operator new(size_t numBytes) {
  ...
  if (!once) {
    ...
    for (int i = 0; i < MAX_PARTICLES - 1; i++) {
      ((Particle2D *)particleArray[i])->next =
          (Particle2D *)particleArray[i + 1];
    }
  ...
  if (numBytes > MAX_PARTICLE_CLASS_SIZE) {
    throw Exception(
        "ERROR: Tried to create a particle with class size %d bytes, but max "
        "particle class object size is %d bytes",
        (int)numBytes, int(MAX_PARTICLE_CLASS_SIZE));
  }
...
void Particle2D::operator delete(void *block) {
  // !TEST! Make sure the particle we re freeing was created using our operator
  // new.
#ifdef _DEBUG
  {
