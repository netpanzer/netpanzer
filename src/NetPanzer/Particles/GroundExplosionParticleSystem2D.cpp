...
//---------------------------------------------------------------------------
GroundExplosionParticleSystem2D::GroundExplosionParticleSystem2D(
    fXYZ pos, iRect bounds, int maxParticleCount, int maxParticleSpeed)
  ...
  int particleCount =
      (rand() % maxParticleCount) + 1;  // max maxParticleCount, min 1
  ...
  // float maxParticleHeight = Physics::getVerticalPosition(maxParticleSpeed,
  // deg2Rad(90), midTime);
  ...
  for (int i = 0; i < particleCount; i++) {
    ...
    // These particles have a hardcoded waitMax and waitMin.  They are the last
    // 2 variables in the following statement.
    // FIXME
