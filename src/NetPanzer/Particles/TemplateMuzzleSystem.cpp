...
TemplateMuzzleSystem::TemplateMuzzleSystem(const fXYZ &pos,
  ...
  if (m.muzzleType == MuzzleSystem::SINGLE) {
    try {
      new VectorPuffParticle2D(pos, direction, m.puffMinScale, m.puffRandScale,
                               m.speedMin, m.speedRand, m.waitMin, m.waitRand,
                               m.lifetimeMin, m.lifetimeRand,
    ...
    try {
      new VectorPuffParticle2D(
          pos, fXYZ(eastDirection.x, 0, eastDirection.y), m.puffMinScale,
          m.puffRandScale, m.speedMin, m.speedRand, m.waitMin, m.waitRand,
          m.lifetimeMin, m.lifetimeRand, MuzzleSystem::windScale);
    } catch (...) {
    }
    ...
    try {
      new VectorPuffParticle2D(
          pos, fXYZ(westDirection.x, 0, westDirection.y), m.puffMinScale,
          m.puffRandScale, m.speedMin, m.speedRand, m.waitMin, m.waitRand,
          m.lifetimeMin, m.lifetimeRand, MuzzleSystem::windScale);
    } catch (...) {
    }
