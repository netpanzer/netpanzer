...
CraterParticle2D::CraterParticle2D(const fXYZ &pos) : Particle2D(pos) {
  ...
  if (isAlive) {
    ...
    craterCache[curCraterIndex].bounds =
        iRect(destPos.x - halfBoundsSize, destPos.y - halfBoundsSize,
              destPos.x + halfBoundsSize, destPos.y + halfBoundsSize);
  ...
  if (MapInterface::craters_fading == 0) {
    ...
    if (age > lifetime) {
      ...
      packedSurface.setDrawModeBlend(&Palette::colorTable4060);
               ...
               lifetime * 0.0f) {  // was 0.6 - still ok if original tileset
      packedSurface.setDrawModeBlend(&Palette::colorTable2080);
    }
  }
