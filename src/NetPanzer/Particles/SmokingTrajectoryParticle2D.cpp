...
//---------------------------------------------------------------------------
SmokingTrajectoryParticle2D::SmokingTrajectoryParticle2D(
    const fXYZ &pos, int maxSpeed, float waitMin, float waitRand,
...
void SmokingTrajectoryParticle2D::sim() {
  ...
  //	{
  //		// Add a new puff of smoke at the current position of each
  //particle. 		iXY randOffset((rand() % 20) - 20, (rand() % 20) - 20); 		pos.x +=
  //randOffset.x; 		zPos  += randOffset.y;
  //
  //		// This is a fudge to make the particle get bigger the higher they
  //are in the air. 		const float deltaY = 100.0f;
  //
  //		if      (particleType == LIGHT_SMOKE_PUFF)
  //		{
  //			new PuffParticle2D(fXYZ(pos.x, pos.y, zPos), LIGHT_SMOKE_PUFF,
  //pos.y / deltaY, 0.2f, smolderMinFPS, smolderRandFPS, smolderLayer,
  //smolderWindScale);
  //		}
  //		else if (particleType == DARK_SMOKE_PUFF)
  //		{
  //			new PuffParticle2D(fXYZ(pos.x, pos.y, zPos), DARK_SMOKE_PUFF,
  //pos.y / deltaY, 0.2f, smolderMinFPS, smolderRandFPS, smolderLayer,
  //smolderWindScale);
  //		}
  //		else if (particleType == DIRT_PUFF)
  //		{
  //			new PuffParticle2D(fXYZ(pos.x, pos.y, zPos), DIRT_PUFF, pos.y /
  //deltaY, 0.2f, smolderMinFPS, smolderRandFPS, smolderLayer,
  //smolderWindScale);
  //		}
  //
  //		waitTime = 0.0f;
