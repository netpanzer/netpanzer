...
class SmolderParticleSystem2D : public ParticleSystem2D {
 public:
  SmolderParticleSystem2D(const fXYZ &pos, const iRect &emitBounds,
                          float lifetime, float waitMin, float waitMax,
 ...
 protected:
  ...
  int waitDiff;    // The difference in the wait max and min in integer (for
                   // rand()).
