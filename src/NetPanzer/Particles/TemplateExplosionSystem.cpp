...
TemplateExplosionSystem::TemplateExplosionSystem(const fXYZ &pos,
  ...
  if (pixMovementValue != 5 && pixMovementValue != 4) {
    try {
  ...
  if (!isFarAway && canHaveSmoke) {
    ...
    //{
    try {
      new FlashParticle2D(pos, e.flashMinScale, e.flashRandScale,
                          e.flashLifetime, ExplosionSystem::flashLayer);
    } catch (...) {
    }
    //}
    ...
    //
    //	new FlashParticle2D(offset, e.flashMinScale, e.flashRandScale,
    //e.flashLifetime, ExplosionSystem::flashLayer);
    // }
  } else {
  ...
  float percentScale =
      Particle2D::getScale(e.percentScaleMin, e.percentScaleRand);
...

  {
    for (int i = 0; i < smallCount; i++) {
      ...
      try {
        new ChunkTrajectoryParticle2D(
            offset, int(maxSpeed), scaleMin, scaleRand, waitMin, waitRand,
            e.minTrajectoryAngle, e.puffType, (rand() % 2) ? true : false,
            isFarAway || fpsLow, canHaveSmoke);
      } catch (...) {
      }
    }
  }
...

  {
    for (int i = 0; i < mediumCount; i++) {
      ...
      try {
        new ChunkTrajectoryParticle2D(pos, int(maxSpeed), scaleMin, scaleRand,
                                      waitMin, waitRand, e.minTrajectoryAngle,
                                      e.puffType, true, isFarAway || fpsLow,
                                      canHaveSmoke);
      } catch (...) {
      }
    }
