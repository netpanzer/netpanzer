...
class ChunkTrajectoryParticle2D : public TrajectoryParticle2D {
 ...
 public:
  ChunkTrajectoryParticle2D(const fXYZ &pos, int maxSpeed, float scaleMin,
                            float scaleRand, float waitMin, float waitRand,
                            int minTrajectoryAngle, PUFF_TYPE particleType,
                            int dieAtMidFlight = 0, int isFarAway = 0,
                            int canHaveSmoke = 1);
