...
#define DEFAULT_UNIT_PROFILES                                            \
  "Manta, Panther1, Titan, Stinger, Bobcat, Bear, Archer, Wolf, Drake, " \
  "Spanzer"
#define DEFAULT_UNITS_STYLES                                               \
  "original, danisch, desert, metro, night, nva, platane, surpat, tiger, " \
  "woodland"
...
enum { _gamemode_skirmish, _gamemode_multiplayer };

enum { _game_session_host, _game_session_join, _game_session_last };
...
enum { _connection_tcpip, _connection_direct_connect, _connection_modem };
...
class GameConfig : public NoCopy {
 public:
  ...
  static NPString*
      game_unit_profiles;  // "," or space separated list of profiles to load
  static NPString* game_unit_spawnlist;  // "," or space separated list with the
                                         // numbers of each unit to spawn with
 ...
 public:
  ...
  static unsigned short getUnitStylesNum() {
    ...
    if (numc > MAX_STYLES_NUM) {
      LOGGER.warning("Too many unit styles provided by user. Max number is %d",
                     MAX_STYLES_NUM);
  ...
  int GetUnitsPerPlayer() const { return game_maxunits / game_maxplayers; }

  int GetTimeLimitSeconds() const { return game_timelimit * 60; }
 ...
 private:
  ...
  PIX colorEnumToPix(int color_enum) const {
    switch (color_enum) {
      case _color_aqua:
        return (Color::unitAqua);
      case _color_yellow:
        return (Color::unitYellow);
      case _color_red:
        return (Color::unitRed);
      case _color_blue:
        return (Color::unitBlue);
      case _color_dark_blue:
        return (Color::unitDarkBlue);
      case _color_light_green:
        return (Color::unitLightGreen);
      case _color_green:
        return (Color::unitGreen);
      case _color_blue_gray:
        return (Color::unitBlueGray);
      case _color_dark_red:
        return (Color::unitDarkRed);
      case _color_black:
        return (Color::unitBlack);
      case _color_dark_green:
        return (Color::unitDarkGreen);
      case _color_white:
        return (Color::unitWhite);
      case _color_light_orange:
        return (Color::unitLightOrange);
      case _color_orange:
        return (Color::unitOrange);
      case _color_gray:
        return (Color::unitGray);
      case _color_dark_gray:
        return (Color::unitDarkGray);
    }  // ** switch
