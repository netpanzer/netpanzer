...
void InfoSocket::onSocketError(UDPSocket *so) {
  (void)so;
  LOGGER.debug("InfoSocket: socket error");
}

void InfoSocket::onDataReceived(UDPSocket *s, const Address &from,
  ...
  while (!(query = qtokenizer.getNextToken()).empty()) {
    ...
    if (query == "status") {
      ...
      LOGGER.debug("InfoSocket:: sending answer [%s][%d]", answer.c_str(),
                   (int)answer.size());
      ...
      LOGGER.info("InfoSocket:: Received echo query of size %lu",
                  (unsigned long)echotoken.size());
      if (echotoken.size())
...
std::string InfoSocket::prepareStatusPacket() {
  std::stringstream s;
  PlayerID playingPlayers = PlayerInterface::countPlayers();
  PlayerID maxPlayers = PlayerInterface::getMaxPlayers();

  s << statusHead << "\\mapname\\" << *GameConfig::game_map << "\\mapcycle\\"
    << *GameConfig::game_mapcycle << "\\mapstyle\\"
    << *GameConfig::game_mapstyle << "\\authentication\\"
    << (GameConfig::server_authentication == false ? 'n' : 'y');
  if (GameConfig::server_authentication == false)
    s << "\\password\\" << (GameConfig::game_gamepass->size() == 0 ? 'n' : 'y');
  else
    s << "\\password\\" << 'n';

  s << "\\numplayers\\" << (int)playingPlayers << "\\maxplayers\\"
    << (int)maxPlayers;
  ...
  s << "\\gamestyle\\" << gameconfig->getGameTypeString()
    << "\\units_per_player\\" << gameconfig->GetUnitsPerPlayer() << "\\time\\"
    << GameManager::getGameTime() / 60 << "\\timelimit\\"
    << GameConfig::game_timelimit << "\\fraglimit\\"
    << GameConfig::game_fraglimit << "\\objectivelimit\\"
  ...
  for (i = 0; i < maxPlayers; ++i) {
    ...
    if (playerState->isActive()) {
      s << "\\player_" << n << "\\" << playerState->getName() << "\\kills_" << n
        << "\\" << playerState->getKills() << "\\deaths_" << n << "\\"
        << playerState->getLosses() << "\\score_" << n << "\\"
        << playerState->getObjectivesHeld() << "\\points_" << n << "\\"
        << playerState->getTotal() << "\\flag_" << n << "\\" << (int)i
...
std::string InfoSocket::prepareFlagPacket(const int flagNum) {
  Uint8 rawflag[20 * 14];
