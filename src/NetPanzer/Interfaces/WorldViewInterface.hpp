...
class WorldViewInterface {
 ...
 public:
  ...
  static inline ViewCamera* getMainCamera() { return main_camera; }
  ...
  static inline void setCameraSize(unsigned long view_size_x,
  ...
  static inline void clientXYtoWorldXY(const iRect& view_win, const iXY& client,
                                       iXY* world) {
    world->x = view_win.min.x + client.x;
    world->y = view_win.min.y + client.y;
  }

  static inline void worldXYtoClientXY(const iRect& view_win, const iXY& world,
                                       iXY* client) {
