...
std::string CommandList[CommandMax] = {
    "/server listcommands", "/server adminlogin",   "/server unitspawnlist",
    "/server unitprofiles", "/server listprofiles", "/server kick",
    "/server baselimit",    "/server gamepass",     "/server map ",
    "/server autokick",     "/server say",          "/server listplayers",
    "/server flagtimer"};
...
void ConsoleInterface::setStdoutPipe(bool on_off) { stdout_pipe = on_off; }

void ConsoleInterface::update(Surface &surface) { update_overlap(surface); }

void ConsoleInterface::postMessage(PIX msgcolor, bool hasFlag, FlagID flag,
                                   const char *format, ...) {
  char temp_str[256];
  char *temp_str_ptr;
...
void ConsoleInterface::update_overlap(Surface &surface) {
  ...
  do {
    ...
    if (line_list[index].visible) visible_count++;
  ...
  if (input_string_active == true) {
    current_line.y =
        bounds.min.y + (line_offset.y * visible_count) + (line_offset.y * 2);
...
void ConsoleInterface::getInputString(char *str) { strcpy(str, inputString); }
...
void ConsoleInterface::addChar(int newChar) {
  ...
  if (cursorPos != length) {
    memmove(&inputString[cursorPos + 1], &inputString[cursorPos],
            length - cursorPos);
    ++length;
  }
...
void ConsoleInterface::addExtendedChar(int newExtendedChar) {
  ...
  switch (newExtendedChar) {
    ...
    case SDLK_DELETE: {
      ...
      memcpy(inputString + cursorPos, inputString + cursorPos + 1,
             strlen(inputString + cursorPos + 1) + 1);
    } break;
    ...
    case SDLK_BACKSPACE: {
      if (cursorPos >= 1) {
        ...
        if (byteCount > 0) {
          memcpy(inputString + cursorPos - 1, inputString + cursorPos,
                 byteCount);
        }
