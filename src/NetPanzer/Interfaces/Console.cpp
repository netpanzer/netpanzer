...
class FileStampStreamBuf : public std::streambuf {
 public:
  FileStampStreamBuf(std::ostream* file) : needstamp(true) {
    stream = &std::cout;
    this->file = file;
  ...
  virtual int overflow(int c) {
    ...
    for (char* p = pbase(); p != pptr(); ++p) {
      if (*p == '\n') {
        ...
        if (file) file->write(chunkstart, p - chunkstart + 1);
        redisplay_prompt();
        if (p < pptr() - 1)
          outputdate();
        else
    ...
    if (file) file->write(chunkstart, pptr() - chunkstart);
...
void Console::initialize() { server = new OFileStampStream(0); }

void Console::shutdown() { delete server; }

void Console::mapSwitch(const std::string& mapname) {
  if (GameConfig::server_logging) {
    ...
    try {
      ...
      LOGGER.warning("Couldn't open logfile '%s': %s", filename.c_str(),
                     e.what());
      return;
    }
  }
