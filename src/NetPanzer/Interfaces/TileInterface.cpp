...
PIX TileInterface::getWorldPixColor(int worldX, int worldY) {
  ...
  if (tileX >= (int)MapInterface::getWidth() ||
      tileY >= (int)MapInterface::getHeight() || tileX < 0 || tileY < 0) {
    LOGGER.warning("query for worldpixcolor outside map (%d,%d)", worldX,
                   worldY);
    return 0;
  }
...
long TileInterface::getWorldPixMovementValue(int worldX, int worldY) {
  ...
  if (tileX >= (int)MapInterface::getWidth() ||
      tileY >= (int)MapInterface::getHeight() || tileX < 0 || tileY < 0) {
    LOGGER.warning("query for worldpixmovement outside map (%d,%d)", worldX,
                   worldY);
    return 0xffff;
  }
  int tileValue = MapInterface::MapValue(tileX, tileY);
