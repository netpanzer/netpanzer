...
class PlayerUnitConfig {
 ...
 public:
  ...
  inline unsigned int getSpawnUnitCount(unsigned char unit_type) const {
    if (unit_type < unit_spawn_list.size()) return unit_spawn_list[unit_type];
    return 0;
  }
  ...
  inline void decrementSpawnUnitCount(unsigned char unit_type) {
    if (unit_type < unit_spawn_list.size()) {
      if ((unit_spawn_list[unit_type] > 0) && (unitTotal() > 1)) {
        unit_spawn_list[unit_type]--;
      }
    }
  }
  ...
  inline char getUnitColor(void) const { return (unit_color); }

  inline void setUnitColor(char color) { unit_color = color; }
};
