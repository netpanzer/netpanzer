...
class UMesgAICommand : public UnitMessage {
 ...
 public:
  ...
  iXY getGoalLoc() const { return iXY(ltoh32(goal_loc_x), ltoh32(goal_loc_y)); }

  UnitID getTargetUnitID() const { return ltoh16(target_id); }
...
class UMesgWeaponHit : public UnitMessage {
 ...
 public:
  void setOwnerUnitID(UnitID id) { owner_id = htol16(id); }

  UnitID getOwnerUnitID() const { return ltoh16(owner_id); }
...
class UMesgEndLifeCycleUpdate : public UnitMessage {
 ...
 public:
  ...
  UnitID getDestroyed() const { return ltoh16(destroyed); }

  UnitID getDestroyer() const { return ltoh16(destroyer); }
} __attribute__((packed));

enum {
  _select_box_allie_visibility,
...
class UMesgSelfDestruct : public UnitMessage {
 public:
  UMesgSelfDestruct() { message_id = _umesg_self_destruct; }
} __attribute__((packed));
