...
void NetPacketDebugger::logMessage(const char* domain,
...
void NetPacketDebugger::logTerminalMessage(std::ostream& log,
  ...
  switch (message->message_id) {
    case _net_message_id_term_unit_cmd: {
      TerminalUnitCmdRequest* unitcmd = (TerminalUnitCmdRequest*)message;
...
void NetPacketDebugger::logAICommand(std::ostream& log,
  ...
  switch (cmd.command) {
    case _command_move_to_loc: {
      log << "move to:" << cmd.getGoalLoc().x << "," << cmd.getGoalLoc().y;
      break;
    }
    ...
    case _command_manual_fire: {
      log << "MFire: " << cmd.getTargetLoc().x << "," << cmd.getTargetLoc().y;
      break;
    }
    default: {
      log << "?";
...
void NetPacketDebugger::logUnitMessage(std::ostream& log,
                                       const NetMessage* message) {
  log << "Unit/";
  switch (message->message_id) {
...
void NetPacketDebugger::logUnitOpcodeMessage(std::ostream& log,
  ...
  //    Uint8* dataend
  //        = dataptr + (message->getSize() -
  //        UnitOpcodeMessage::getHeaderSize());
