...
NetworkClient::NetworkClient(void) : NetworkInterface(), clientsocket(0) {
...
void NetworkClient::onClientConnected(ClientSocket *s) {
  (void)s;
...
void NetworkClient::onClientDisconnected(ClientSocket *s, const char *msg) {
  if (NetworkState::status == _network_state_bot)  // client bot only
  {
    ...
    if (!Desktop::getVisible("LoadingView")) {
      DisconectedView *dv =
          (DisconectedView *)Desktop::getView("DisconectedView");
      dv->setMsg(msg);
...
bool NetworkClient::joinServer(const NPString &server_name,
                               const NPString &password) {
  if (clientsocket) delete clientsocket;
  clientsocket = 0;
...
void NetworkClient::sendMessage(const NetMessage *message, size_t size) {
  if (!clientsocket) {
    EnqueueIncomingPacket(message, size, PlayerInterface::getLocalPlayerIndex(),
                          0);
  } else {
...
void NetworkClient::sendRemaining() {
  if (clientsocket) clientsocket->sendRemaining();
}
