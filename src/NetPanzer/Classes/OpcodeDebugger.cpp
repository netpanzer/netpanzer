...
void OpcodeDebugger::logOpcode(std::ostream& log, UnitOpcode* opcode) {
  log << " UNIT:" << opcode->getUnitID() << " FL:" << (int)opcode->flags << " ";
  switch (opcode->opcode) {
    ...
    case _UNIT_OPCODE_TURRET_TRACK_TARGET: {
      TurretTrackTargetOpcode* tracktcode = (TurretTrackTargetOpcode*)opcode;
    ...
    case _UNIT_OPCODE_UPDATE_STATE: {
      UpdateStateUnitOpcode* updatecode = (UpdateStateUnitOpcode*)opcode;
