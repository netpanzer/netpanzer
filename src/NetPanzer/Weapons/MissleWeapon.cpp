...
MissleWeapon::MissleWeapon(UnitID owner, unsigned short owner_type_id,
                           unsigned short size, unsigned short damage,
                           iXY &start, iXY &end)
  ...
  do {
    switch (fsmFlight_state) {
      ...
      case _fsmFlight_in_flight: {
        ...
        // **  Particle Shit
        ParticleInterface::addMissleFlightPuff(
            location, direction, curWait, totalWait, Weapon::owner_type_id);
...
void MissleWeapon::updateStatus(void) {
  ...
  groundLight.setWorldPos(location.x + thrustOffset.x,
                          location.y + thrustOffset.y + heightFromGround);
