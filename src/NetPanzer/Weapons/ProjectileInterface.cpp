...
void ProjectileInterface::removeAll() {
  ...
  for (i = projectiles.begin(); i != projectiles.end(); ++i) delete *i;

  projectiles.clear();
}

void ProjectileInterface::resetLogic() { removeAll(); }
...
void ProjectileInterface::newProjectile(unsigned short projectile_type,
  ...
  if (projectile_type == Weapon::_quad_missile) {
    // The following code launches 4 missles spaced out evenly like they would
    // be in a missle bay.
    iXY startPos;
    iXY endPos;
    ...
    // The following code launches 2 missles spaced out evenly like they would
    // be in a missle bay.
    iXY startPos;
    iXY endPos;
