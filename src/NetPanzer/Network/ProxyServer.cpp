...
void ProxyServer::makeBase64(std::string &base64, const std::string &str) {
  ...
  for (auth_ptr = str.c_str(); *auth_ptr; auth_ptr += 3) {
    ...
    auth_ptr_dest[1] =
        base64_chars[((auth_ptr[0] & 0x3) << 4) | ((auth_ptr[1] & 0xf0) >> 4)];
    if (auth_ptr[1] == 0) {
      base64 += auth_ptr_dest;
      break;
    }
    auth_ptr_dest[2] =
        base64_chars[((auth_ptr[1] & 0xf) << 2) | ((auth_ptr[2] & 0xc0) >> 6)];
    if (auth_ptr[2] == 0) {
      base64 += auth_ptr_dest;
      break;
    }
...
void ProxyServer::getProxyConnect(std::stringstream &buffer,
...
void ProxyServer::sendProxyConnect(network::TCPSocket &socket,
                                   const std::string &serveraddress) {
