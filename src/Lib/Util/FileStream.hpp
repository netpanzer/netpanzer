...
class IFileStreambuf : public std::streambuf {
 public:
  IFileStreambuf(filesystem::ReadFile* newfile) : file(newfile) {
    // start reading
    underflow();
  }

  ~IFileStreambuf() { delete file; }

 protected:
  virtual int underflow() {
    if (file->isEOF()) return traits_type::eof();
...
class OFileStreambuf : public std::streambuf {
 public:
  OFileStreambuf(filesystem::WriteFile* newfile) : file(newfile) {
 ...
 protected:
  virtual int overflow(int c) {
    ...
    try {
      ...
      if (c != traits_type::eof()) file->write8(c);
  ...
  virtual int sync() { return overflow(traits_type::eof()); }
