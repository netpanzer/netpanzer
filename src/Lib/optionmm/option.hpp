...
class basic_option {
 ...
 public:
  ...
  basic_option(char s, const std::string& l, const std::string& h);
  /// DTOR
...
template <typename Type, bool argument = true, bool multivalue = true,
...
class option : public basic_option {
 ...
 public:
  ...
  option(char s, const std::string& l, const std::string& h, value_type d);
  /** Destructor. */
  virtual ~option() {}
...
template <typename Type, bool arg, bool multi, typename Convert,
          typename Toggle>
...
inline Type option<Type, arg, multi, Convert, Toggle>::value(size_t i) const {
  if (!many_values() || i >= _values.size()) return _values[0];
  return _values[i];
}
