...
class command_line {
 ...
 public:
  ...
  command_line(const std::string title, const std::string& version,
               const std::string& copy, const std::string& usage, int& argc,
               char** argv, bool fail = true);
...
struct toggle_value<bool> {
  ...
  inline void operator()(std::vector<bool>::reference x) { x.flip(); }
#endif
};
