...
//---------------------------------------------------------------------------
void ColorTable::createBrightenFilter(const char* filename,
...
void ColorTable::createDarkenFilter(const char* filename, float fudgeValue) {
  ...
  for (int y = 0; y < 256; y++) {
    for (int x = 0; x < 256; x++) {
      ...
      setColor(curOffset,
               Palette::findNearestColor(curColor.r, curColor.g, curColor.b));
    }
  }
...
// create
//---------------------------------------------------------------------------
void ColorTable::create(const int color1Percent, const int color2Percent,
                        const char* filename) {
  init(256 * 256);
  ...
  for (unsigned index = 0; index < 256; index++) {
    ...
    for (unsigned indexPic = 0; indexPic < 256; indexPic++) {
      ...
      //            SDL_Color curColor((Uint8) (color1 * col.r   + color2 *
      //            colPic.r),
      //                              (Uint8) (color1 * col.g + color2 *
      //                              colPic.g), (Uint8) (color1 * col.b  +
      //                              color2 * colPic.b));
      SDL_Color curColor;
...
void ColorTable::loadTable(const char* filename) {
  try {
    ...
    throw Exception("couldn't load colortable '%s': %s", filename, e.what());
  }
...
void ColorTable::saveTable(const char* filename) const {
  try {
    ...
    throw Exception("error while writing to file '%s': %s", filename, e.what());
  }
}  // end ColorTable::saveTable

// createTrans0
//---------------------------------------------------------------------------
void ColorTable::createTrans0(const int& color1Percent,
                              const int& color2Percent, const char* filename) {
  init(256 * 256);
  ...
  for (unsigned index = 0; index < 256; index++) {
    ...
    for (unsigned indexPic = 0; indexPic < 256; indexPic++) {
      ...
      //            SDL_Color curColor((int) (color1 * col.r   + color2 *
      //            colPic.r),
      //                              (int) (color1 * col.g + color2 *
      //                              colPic.g), (int) (color1 * col.b  + color2
      //                              * colPic.b));
      SDL_Color curColor;
...
void ColorTable::createLightDarkFilter(const char* filename) {
  ...
  for (int y = 0; y < 256; y++) {
    ...
    for (x = 129; x < 256; x++) {
      curOffset = x + (y << 8);
      curRed =
          Palette::color[y].r + ((255 - Palette::color[y].r) * (x - 128) / 127);
      curGreen =
          Palette::color[y].g + ((255 - Palette::color[y].g) * (x - 128) / 127);
      curBlue =
          Palette::color[y].b + ((255 - Palette::color[y].b) * (x - 128) / 127);
