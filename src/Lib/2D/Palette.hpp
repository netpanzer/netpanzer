...
class Palette {
 ...
 public:
  ...
  // static ColorTable earth256;  // Earth colors from sea 2 land 2 mountain 2
  // snow.
  ...
  static SDL_Color
      color[PALETTE_LENGTH];  // This has any brightness value added to it.
  static SDL_Color originalColor[PALETTE_LENGTH];  // This is the original
                                                   // source loaded values.
  ...
  static void ramp(SDL_Color table[], int startRGB, int r1, int g1, int b1,
                   int endRGB, int r2, int g2, int b2);
  static void loadACT(const std::string &filename);
  static Uint8 findNearestColor(int r, int g, int b,
                                const bool &ignoreIndexZero = false);
